...
// ===========================================================================
// VECTORS. one shape, five jobs.
// ===========================================================================

// the alias never changes the math -- it says what a given pair of numbers is
// FOR.
...
// sunshine2k and learnopengl both write this a ∙ b and call it the dot product.
...
// ===========================================================================
// BOXES. everything on the court is one of these.
// ===========================================================================
...
// ===========================================================================
// THE COURT. a box too, so the same math fences the play field.
// ===========================================================================
...
// chase -- every test in this file is then point-vs-box, never box-vs-box.
...
// mid-court, until resetBall() serves it
...
// ===========================================================================
// THE PADDLE. it follows the mouse.
// ===========================================================================
...
// sunshine2k's reflection formula, letter for letter:
...
// ===========================================================================
// EVERY FRAME. move first, then show the result.
// ===========================================================================

// everything that moves runs here, in this order, once per frame.
function update(elapsed: number) {
  // the mouse asks for a center; this hands back the nearest allowed
  ...
  // the ball waits for the first click. the paddle does not.
  if (!running) return;

  // Velocity -> Vector: px/s * s = px. the only use of elapsed.
  const step = scale(ballVelocity, elapsed);
  ball = movedBoxBy(ball, step); // move first, then undo any overlap below

  // three walls. the player's end has a paddle instead.
  bounceOffWall(DOWN);
  bounceOffWall(UP);
  bounceOffWall(RIGHT);
  ...
  // the GROWN court, so the ball is gone rather than just going
...
function draw() {
  // the wipe. everything below is drawn onto a fresh black court.
  ctx.fillStyle = "black";
  ...
  // the score. fillText reuses the fillStyle set just above.
  ctx.font = "20px monospace";
  ctx.fillText("Score: " + score, 20, 30); // 20 across, 30 down from the top

  // the hint, until the first click
  if (!running) {
...
// serve once at load, so the very first frame already has a ball to draw
resetBall();
